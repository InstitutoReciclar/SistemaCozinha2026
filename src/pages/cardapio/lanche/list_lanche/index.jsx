...
import { ArrowLeft, Download, Filter, FilterX, Calendar, User, Cookie, Edit3, Search, Clock } from "lucide-react"
...
const ConsultaCardapioLanche = () => {
  ...
  useEffect(() => {
    const cardapioRef = ref(db, "cardapiosAprovadosPorTipo/LanchedaTarde")
    onValue(cardapioRef, (snapshot) => {
      const data = snapshot.val()
      if (data) {const lista = Object.entries(data).map(([key, value]) => ({ id: key, ...value,})); 
      setCardapios(lista)} 
      else {setCardapios([])} 
      setLoading(false)
    })
  ...
  const filtrarCardapios = () => {
    return cardapios.filter((item) => {
      ...
      const matchesSearch =  !searchTerm || item.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
        Object.values(item.composicoes || {}).some((dados) => dados.nutricionista?.nome?.toLowerCase().includes(searchTerm.toLowerCase()),)
  ...
  const exportarExcel = () => {
    const dadosExportar = filtrarCardapios().flatMap((cardapio) =>
      Object.entries(cardapio.composicoes || {}).flatMap(([semana, dados]) =>
        Object.entries(dados.cardapio || {}).map(([secao, dias]) => ({ID: cardapio.id, Semana: semana, Nutricionista: dados.nutricionista?.nome || "", 
          CRN3: dados.nutricionista?.crn3 || "", Início: cardapio.periodo?.inicio || "", Fim: cardapio.periodo?.fim || "", Seção: secao, ...dias,})),
      ),
    )

    const worksheet = XLSX.utils.json_to_sheet(dadosExportar)
    const workbook = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(workbook, worksheet, "Cardápio Lanche")
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" })
    const blob = new Blob([excelBuffer], { type: "application/octet-stream" })
    saveAs(blob, "cardapioLancheDaTarde.xlsx")
  }
  ...
  const handleBlur = async () => {
    ...
    const caminho = `cardapiosAprovadosPorTipo/LanchedaTarde/${cardapioId}/composicoes/${semana}/cardapio/${secao}/${dia}`
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex items-center justify-between mb-8">
          ...
          <div className="text-center">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Consulta de Cardápios - Lanche da Tarde</h1>
            <Badge variant="secondary" className="text-sm"><Cookie className="h-3 w-3 mr-1" />{cardapios.length} cardápios cadastrados</Badge>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <Card className="mb-6">
            <CardHeader> <CardTitle className="flex items-center text-lg"> <Filter className="h-5 w-5 mr-2" />Filtros de Consulta </CardTitle></CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div className="flex items-end">
                  <Button onClick={limparConsulta} variant="outline" className="w-full"><FilterX className="h-4 w-4 mr-2" />Limpar</Button>
                </div>
                <div className="flex items-end">
                  <Button onClick={exportarExcel} className="w-full bg-green-600 hover:bg-green-700" disabled={cardapiosFiltrados.length === 0}><Download className="h-4 w-4 mr-2" /> Excel</Button>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          {cardapiosFiltrados.length === 0? "Nenhum cardápio encontrado" : `${cardapiosFiltrados.length} cardápio(s) encontrado(s)`}</p>
                ...
                <Card className="overflow-hidden">
                  <CardHeader className="bg-gradient-to-r from-purple-50 to-pink-50">
                    <div className="flex items-center justify-between">
                      <div>
                        <CardTitle className="flex items-center text-lg"><Cookie className="h-5 w-5 mr-2 text-purple-600" />Cardápio #{item.id.slice(-8)}</CardTitle>
                        ...
                        <div className="overflow-x-auto">
                          <div className="min-w-[700px]">
                            <div className="grid grid-cols-6 gap-2 mb-2">
                              ...
                              {diasSemana.map((dia) => (<div key={dia} className="font-semibold text-sm text-gray-700 p-3 bg-purple-50 rounded-md text-center">{dia}</div>))}
                            </div>
                            ...
                            {Object.entries(dados.cardapio || {}).map(([secao, dias], index) => (
                              <div key={index} className="grid grid-cols-6 gap-2 mb-2">
                                ...
                                {diasSemana.map((dia) => (
                                  <div key={dia} className="p-2 border rounded-md cursor-pointer hover:bg-pink-50 transition-colors min-h-[40px] flex items-center" onDoubleClick={() => handleDoubleClick(item.id, semana, secao, dia, dias[dia])}>
                                    {celulaEditando.cardapioId === item.id && celulaEditando.semana === semana && celulaEditando.secao === secao && celulaEditando.dia === dia ? (
                                      <Input className="h-8 text-sm" value={celulaEditando.valor} onChange={(e) => setCelulaEditando({ ...celulaEditando, valor: e.target.value })}
                                        onBlur={handleBlur} onKeyDown={(e) => {if (e.key === "Enter") handleBlur()}} autoFocus />
