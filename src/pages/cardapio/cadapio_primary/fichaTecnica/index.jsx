...
import { FileText, Search, ArrowLeft, ChefHat, BookOpen, Sparkles } from "lucide-react"
import { Card, CardContent } from "@/components/ui/card"
import { Badge } from "@/components/ui/badge"

const cardData = [
  {
    link: "/cadatro-receita",
    icon: FileText,
    title: "Cadastrar",
    description: "Criar nova ficha técnica de receita",
  ...
  {
    link: "/consultar-receita",
    icon: Search,
    title: "Consultar",
    description: "Visualizar fichas técnicas existentes",
    ...
    <Card className="group relative overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 bg-white/10 backdrop-blur-sm border border-white/20">
      <CardContent className="p-8 h-full flex flex-col items-center text-center relative">
        ...
        <div className="relative z-10">
          ...
          <p className="text-white/70 group-hover:text-white/80 transition-colors">Retornar ao cardápio</p>
        </div>
        {/* Action indicator */}
        <div className="relative z-10 mt-6 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-6">
        ...
        <div className="text-center mb-16 max-w-4xl">
          <div className="flex items-center justify-center mb-6">
            <ChefHat className="h-10 w-10 text-emerald-400 mr-4 animate-pulse" />
            <Badge className="bg-white/10 text-white border-white/20 backdrop-blur-sm px-6 py-2 text-sm">Gestão de Receitas</Badge>
            <BookOpen className="h-10 w-10 text-blue-400 ml-4 animate-pulse" />
          </div>
          <h1 className="text-4xl md:text-5xl lg:text-7xl font-bold bg-gradient-to-r from-white via-emerald-200 to-blue-200 bg-clip-text text-transparent mb-6 leading-tight">Ficha Técnica</h1>
          <p className="text-xl text-white/70 max-w-3xl mx-auto leading-relaxed">Gerencie e organize todas as fichas técnicas das suas receitas de forma profissional e detalhada</p>
        </div>
        {/* Cards Grid */}
        <div className="w-full max-w-6xl">
          ...
          <div className="flex justify-center">
            <div className="w-full max-w-sm">
              <Link to="/cardapio" className="block"><BackCard /></Link>
            </div>
          </div>
        </div>
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl w-full">
          <div className="text-center p-6 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 group hover:bg-white/10 transition-all duration-300">
            <div className="w-12 h-12 bg-gradient-to-br from-emerald-500 to-teal-500 rounded-xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300"><FileText className="h-6 w-6 text-white" /></div>
            <div className="text-2xl font-bold text-emerald-400 mb-2">Criar</div>
            <div className="text-white/70 text-sm">Novas fichas técnicas</div>
          </div>

          <div className="text-center p-6 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 group hover:bg-white/10 transition-all duration-300">
            <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-500 rounded-xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300"><Search className="h-6 w-6 text-white" /></div>
            <div className="text-2xl font-bold text-blue-400 mb-2">Buscar</div>
            <div className="text-white/70 text-sm">Receitas existentes</div>
          </div>

          <div className="text-center p-6 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 group hover:bg-white/10 transition-all duration-300">
            <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-pink-500 rounded-xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300"><BookOpen className="h-6 w-6 text-white" /></div>
            <div className="text-2xl font-bold text-purple-400 mb-2">Organizar</div>
            <div className="text-white/70 text-sm">Sistema completo</div>
          </div>
        </div>

        {/* Quick Actions */}
        <div className="mt-12 flex flex-wrap justify-center gap-4">
          <Badge className="bg-emerald-500/20 text-emerald-300 border-emerald-500/30 px-4 py-2 hover:bg-emerald-500/30 transition-colors cursor-pointer">📝 Receitas Rápidas</Badge>
          <Badge className="bg-blue-500/20 text-blue-300 border-blue-500/30 px-4 py-2 hover:bg-blue-500/30 transition-colors cursor-pointer">🔍 Busca Avançada</Badge>
          ...
          <Badge className="bg-pink-500/20 text-pink-300 border-pink-500/30 px-4 py-2 hover:bg-pink-500/30 transition-colors cursor-pointer">⭐ Favoritas</Badge>
        </div>

        {/* Footer info */}
        <div className="mt-12 text-center"> <p className="text-white/50 text-sm">Instituto Reciclar - Sistema de Fichas Técnicas</p></div>
      </div>
    </div>
  )
}
