...
import { ArrowLeft, Save, Send, Calendar, User, Coffee, Clock } from "lucide-react"
...
const CadastroCardapioCafe = () => {
  ...
  const salvarRascunho = async () => {
    try {
      const dados = {tipo: "CafeDaManha", dataAtualizacao: new Date().toISOString(), dadosNutri, composicoes,}
  ...
  const enviarParaAprovacao = async () => {
    ...
    const dados = {
      ...
      tipo: "CafeDaManha",
      periodo: { inicio: dadosNutri[0].dataInicio, fim: dadosNutri[4].dataFim,},composicoes: {},
    }
    ...
    try {
      ...
      setDadosNutri(criarDadosNutriInicial()); setComposicoes(criarComposicoesInicial())
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-center">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Cadastro de Cardápio - Café da Manhã</h1>
            <Badge variant="secondary" className="text-sm"><Coffee className="h-3 w-3 mr-1" />5 Semanas de Planejamento</Badge>
          </div>
          <div className="w-20" />
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <Button onClick={enviarParaAprovacao} size="lg" className="bg-orange-600 hover:bg-orange-700 min-w-[200px]"><Send className="h-4 w-4 mr-2" />Enviar para Aprovação</Button>
        </div>
