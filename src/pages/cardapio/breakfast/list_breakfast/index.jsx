...
import { ArrowLeft, Download, Filter, FilterX, Calendar, User, Coffee, Edit3, Search, Clock } from "lucide-react"
...
const ConsultaCardapioCafe = () => {
  ...
  useEffect(() => {
    const cardapioRef = ref(db, "cardapiosAprovadosPorTipo/CafedaManha")
  ...
  const filtrarCardapios = () => {
    return cardapios.filter((item) => {
      ...
      const matchesSearch = !searchTerm || item.id.toLowerCase().includes(searchTerm.toLowerCase()) || Object.values(item.composicoes || {}).some((comp) =>comp.nutricionista?.nome?.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  const exportarExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Cardápio Café")
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" })
    const blob = new Blob([excelBuffer], { type: "application/octet-stream" })
    saveAs(blob, "cardapioCafeDaManha.xlsx")
  }
  ...
  const handleBlur = async () => {
    ...
    const caminho = `cardapiosAprovadosPorTipo/CafedaManha/${cardapioId}/composicoes/${semana}/cardapio/${secao}/${dia}`
    await update(ref(db), { [caminho]: String(valor) })
    setCelulaEditando({})
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-orange-100">
        <div className="text-center">
          ...
          <p className="text-gray-700">Carregando cardápios...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-100 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}className="flex items-center justify-between mb-8">
          ...
          <div className="text-center">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Consulta de Cardápios - Café da Manhã</h1>
            <Badge variant="secondary" className="text-sm"><Coffee className="h-3 w-3 mr-1" />{cardapios.length} cardápios cadastrados</Badge>
          </div>
          <div className="w-20" />
        </motion.div>
        {/* Filtros */}
                ...
                <Card className="overflow-hidden">
                  <CardHeader className="bg-gradient-to-r from-orange-50 to-amber-50">
                    <div className="flex items-center justify-between">
                      <div>
                        <CardTitle className="flex items-center text-lg"><Coffee className="h-5 w-5 mr-2 text-orange-600" />Cardápio #{item.id.slice(-8)}</CardTitle>
                      ...
                      <div key={semana} className="mb-8 last:mb-0">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className="flex items-center text-sm text-gray-600">
                            <User className="h-4 w-4 mr-1" />{dados.nutricionista?.nome} | CRN3: {dados.nutricionista?.crn3}
                          </div>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                          <div className="min-w-[700px]">
                            <div className="grid grid-cols-6 gap-2 mb-2">
                              ...
                              {diasSemana.map((dia) => (<div key={dia} className="font-semibold text-sm text-gray-700 p-3 bg-orange-50 rounded-md text-center">{dia}</div>))}
                            </div>
                            {Object.entries(dados.cardapio || {}).map(([secao, dias], index) => (
                              <div key={index} className="grid grid-cols-6 gap-2 mb-2">
                                <div className="bg-orange-50 p-3 rounded-md font-medium text-sm flex items-center border border-orange-200">{secao}</div>
                                {diasSemana.map((dia) => (
                                  <div key={dia} className="p-2 border rounded-md cursor-pointer hover:bg-amber-50 transition-colors min-h-[40px] flex items-center" onDoubleClick={() => handleDoubleClick(item.id, semana, secao, dia, dias[dia])}>
                                    {celulaEditando.cardapioId === item.id && celulaEditando.semana === semana && celulaEditando.secao === secao && celulaEditando.dia === dia ? (
                                    <Input className="h-8 text-sm" value={celulaEditando.valor} onChange={(e) =>setCelulaEditando({ ...celulaEditando, valor: e.target.value })}  onBlur={handleBlur} onKeyDown={(e) => { if (e.key === "Enter") handleBlur()}} autoFocus/>
